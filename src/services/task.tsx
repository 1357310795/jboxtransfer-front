...
//获取jbox链接
export async function taskJboxLink(id: number) {
  try {
    const resp = await axios.get<OutputDto<string>>(
      baseurl + '/task/jboxlink' + `?id=${id}`,
      {withCredentials : true}
    )
    ...
    var errmessage = `获取旧云盘链接失败：${error}`
    throw errmessage;
  }
};

//获取tbox链接
export async function taskTboxLink(id: number) {
  try {
    const resp = await axios.get<OutputDto<string>>(
      baseurl + '/task/tboxlink' + `?id=${id}`,
      {withCredentials : true}
    )
    ...
    var errmessage = `获取新云盘链接失败：${error}`
    throw errmessage;
  }
};
